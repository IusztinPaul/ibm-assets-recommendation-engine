    ...
    if sol_1_dict_ == sol_1_dict:
        ...
        for k, v in sol_1_dict.items():
            if sol_1_dict_[k] != sol_1_dict[k]:
                print(
                    "Oops! It looks like the value associated with: {} wasn't right. Try again.  "
                    "It might just be the datatype.  All of the values should be ints except the article_id should "
                    "be a string.  "
                    "Let each row be considered a separate user-article interaction.  "
                    ...
                    "However, this is hard to catch without mapping first!".format(
                        k)
                )
